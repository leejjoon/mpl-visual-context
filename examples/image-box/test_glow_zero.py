...
for p in [p1, p2]:
    ...
    alpha_max = 0.3
    bbox_image_up = AlphaGradient(
        f"0 ^ {alpha_max}", bbox=get_bbox_up, axes=ax, coords="data"
    )
    bbox_image_down = AlphaGradient(
        f"{alpha_max} ^ 0.", bbox=get_bbox_down, axes=ax, coords="data"
    )
