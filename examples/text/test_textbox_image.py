...
ax.set_title("Title", size=20, bbox=dict(boxstyle="round", pad=0.5))
...
if True:
    from mpl_visual_context.image_box_effect import (
        GradientBboxImage,
        ColorBboxAlpha,
        ImageClipEffect,
    )
    ...
    bbox_image = GradientBboxImage(
        "right", alpha="right", extent=[-0.3, 0, 1.3, 1], coords=bp1, cmap=cmap
    )
    ...
    cmap = plt.get_cmap()
    bbox_image = GradientBboxImage("right", alpha="up", coords=bp2, cmap=cmap)
    pe = [ImageClipEffect(bbox_image, ax=ax, clip_box=None)]
