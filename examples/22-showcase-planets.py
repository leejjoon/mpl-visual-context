...
skip_method = ['Pulsation Timing Variations',
               'Pulsar Timing',
               'Orbital Brightness Modulation']

planets = (sns.load_dataset("planets")
           .query(f"method not in {skip_method}"))
...
ax.xaxis.label.set_path_effects([tick_glow,
                                 pe.FillOnly()])
...
for l, o in zip(ind, yoffsets):
    t = ax.annotate(l, (0, o),
                    ...
                    zorder=10)
    t.set_path_effects([pe.GCModify(linewidth=3) | pe.StrokeColor("w"),
                        pe.FillOnly()])
...
bbox_image = ImageBox("right", alpha=0.5,
                      extent=[0, 0, 1, 1],
...
for p in polys:
    bbox_image = ImageBox("right", alpha="0. ^ 0.5",
                          extent=[0, 0, 1, 1],
                          coords=("axes fraction", p),
                          cmap=cmap)
    pel = [pe.FillImage(bbox_image, ax=ax, clip_box=None)
           | pe.ImageEffect(ie.Pad(10) | ie.LightSource(erosion_size=10,
                                                        gaussian_size=20,
                                                        azdeg=200,
                                                        blend_mode="soft")),
           pe.GCModify(alpha=0.5) | pe.Partial(0.5, 1.) | pe.Open()
           | pe.StrokeOnly(),
           ]
